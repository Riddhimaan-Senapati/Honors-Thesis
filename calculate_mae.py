...
def process_file(filepath: str) -> Tuple[float, float, float, int, int, int, int]:
    """Process a single JSON file and return MAE metrics."""
    ...
    if not valid_results:
        return float('nan'), float('nan'), float('nan'), 0, 0, 0, len(all_results)
    
    # Overall MAE
    overall_mae = calculate_mae(ground_truth, llm_scores)
    
    # MAE for ground truth = 0 (only valid experiments)
    ...
    # MAE for positive non-zero ground truth (only valid experiments)
    ...
    return overall_mae, gt_zero_mae, gt_positive_mae, len(valid_results), len(gt_zero_pairs), len(gt_positive_pairs), len(all_results)
...
def main():
    ...
    for file in sorted(json_files):
        try:
            overall_mae, gt_zero_mae, gt_positive_mae, valid_count, gt_zero_count, gt_positive_count, total_count = process_file(file)
